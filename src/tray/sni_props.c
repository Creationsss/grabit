...
static void append_variant_path(DBusMessageIter *parent, const char *path) {
	DBusMessageIter v;
...
static void append_variant_tooltip(DBusMessageIter *parent, const struct sni_props *p) {
	...
	const char *body = p->tooltip_body ? p->tooltip_body : "";
...
bool gsni_append_property_value(DBusMessageIter *parent, const char *name,
	...
	if (strcmp(name, "Category") == 0) {
		gsni_append_variant_s(parent, p->category);
		return true;
	}
	if (strcmp(name, "Id") == 0) {
		gsni_append_variant_s(parent, p->id);
		return true;
	}
	if (strcmp(name, "Title") == 0) {
		gsni_append_variant_s(parent, p->title);
		return true;
	}
	if (strcmp(name, "Status") == 0) {
		gsni_append_variant_s(parent, p->status);
		return true;
	}
	if (strcmp(name, "IconName") == 0) {
		gsni_append_variant_s(parent, p->icon_name);
		return true;
	}
	if (strcmp(name, "OverlayIconName") == 0) {
		gsni_append_variant_s(parent, p->overlay_icon_name);
		return true;
	}
	if (strcmp(name, "AttentionIconName") == 0) {
		gsni_append_variant_s(parent, p->attention_icon_name);
		return true;
	}
	...
	if (strcmp(name, "ToolTip") == 0) {
		append_variant_tooltip(parent, p);
		return true;
	}
	if (strcmp(name, "Menu") == 0) {
		append_variant_path(parent, MENU_PATH);
		return true;
	}
	return false;
}
