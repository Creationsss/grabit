...
static const char *const G_SERVICES[] = {
...
static bool key_namespace_exists(const char *input) {
	...
	for (size_t g = 0; KEY_GROUPS[g]; g++) {
		for (size_t i = 0; KEY_GROUPS[g][i]; i++) {
			const char *k = KEY_GROUPS[g][i];
			if (strncmp(k, input, n) == 0 && k[n] == '.') return true;
		}
	}
	return false;
}
...
void cfg_help_print_all_keys(void) {
	...
	puts("settable config keys (`=` marks one you have set; run `grabit set <key>` "
		 "for values):");
	for (size_t g = 0; KEY_GROUPS[g]; g++) {
		if (KEY_GROUPS[g] == G_SERVICES) continue;
		puts("");
		for (size_t i = 0; KEY_GROUPS[g][i]; i++) {
	...
	for (size_t i = 0; G_SERVICES[i]; i++)
		if (!strstr(G_SERVICES[i], ".auth")) print_key_line(G_SERVICES[i], NULL);
